...
const PlatformSignUp = () => {
  ...
  const handleSignUp = async (data: SignUpRequest) => {
    try {
      ...
      const result = await signUp(data, true) // true = platform admin signup

      if (!result.success) {
        setError(result.error || 'Platform admin sign up failed. Please try again.')
      }
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'An unexpected error occurred. Please try again.'
      setError(errorMessage)
      console.error('Platform admin signup error:', err)
    } finally {
      setLoading(false)
    }
  }
              ...
              <Card className="border-0 shadow-lg">
                <CardBody className="p-5">
                  <div className="text-center">
                    ...
                    <p className="text-muted">Create your platform administrator account. Authorized personnel only.</p>
                  </div>
                  ...
                  <form onSubmit={handleSubmit(handleSignUp)} className="mt-4">
                    ...
                    <div className="mb-1 text-center d-grid">
                      <button
                        ...
                        disabled={loading}
                      >
                        {loading ? 'Creating admin account...' : 'Platform Admin Sign Up'}
                      </button>
                    </div>
                  </form>
                </CardBody>
              ...
              <Row className="mt-4">
                <Col xs={12} className="text-center">
                  <p className="text-white text-opacity-75 mb-2">
                    Already have a platform admin account?{' '}
                  ...
                  <p className="text-white text-opacity-50 mb-0">
                    <Link href="/auth/sign-up" className="text-decoration-none text-white text-opacity-75">
                      ← Create Org User Account Instead
                    </Link>
                  </p>
                </Col>
              </Row>
