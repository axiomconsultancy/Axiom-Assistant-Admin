...
const ContactSupportPage = () => {
    ...
    const STATUS_CONFIG = {
        open: { bg: '#E3F2FD', color: '#1976D2', icon: 'solar:info-circle-bold', label: 'Open' },
        in_progress: { bg: '#FFF3E0', color: '#F57C00', icon: 'solar:settings-bold', label: 'In Progress' },
        waiting_on_user: { bg: '#FFFDE7', color: '#FBC02D', icon: 'solar:clock-circle-bold', label: 'Waiting' },
        resolved: { bg: '#E8F5E9', color: '#2E7D32', icon: 'solar:check-circle-bold', label: 'Resolved' },
        closed: { bg: '#F5F5F5', color: '#757575', icon: 'solar:lock-password-bold', label: 'Closed' }
    }

    const PRIORITY_CONFIG = {
        low: { bg: '#E8F5E9', color: '#2E7D32', icon: 'solar:check-circle-bold', label: 'LOW' },
        ...
        urgent: { bg: '#FFEBEE', color: '#C62828', icon: 'solar:danger-triangle-bold', label: 'URGENT' }
    }

    const TYPE_CONFIG = {
        bug: { bg: '#FFEBEE', color: '#C62828', icon: 'solar:bug-bold', label: 'Bug' },
        billing: { bg: '#FFF3E0', color: '#F57C00', icon: 'solar:bill-list-bold', label: 'Billing' },
        feature_request: { bg: '#E1F5FE', color: '#0288D1', icon: 'solar:star-bold', label: 'Feature' },
        technical: { bg: '#E3F2FD', color: '#1976D2', icon: 'solar:settings-bold', label: 'Technical' },
        other: { bg: '#F5F5F5', color: '#757575', icon: 'solar:question-circle-bold', label: 'Other' }
    }
    ...
    const columns: DataTableColumn<SupportTicket>[] = useMemo(
        () => [
            ...
            },
            {
                ...
                width: 150,
                render: (ticket) => {
                    const config = TYPE_CONFIG[ticket.type as keyof typeof TYPE_CONFIG] || TYPE_CONFIG.other
                    return (
                        <div className="d-flex align-items-center gap-2">
                            <div
                                className="rounded-circle d-flex align-items-center justify-content-center"
                                style={{ width: 24, height: 24, background: config.bg, color: config.color }}
                            >
                                <IconifyIcon icon={config.icon} width={14} height={14} />
                            </div>
                            <span className="fw-semibold" style={{ fontSize: '0.85rem', color: config.color }}>
                                {config.label}
                            </span>
                        </div>
                    )
            ...
            },
            {
                ...
                width: 120,
                render: (ticket) => {
                    const config = PRIORITY_CONFIG[ticket.priority as keyof typeof PRIORITY_CONFIG] || PRIORITY_CONFIG.low
                    return (
                        <div className="d-flex align-items-center gap-2">
                            {/* <div
                                className="rounded-circle d-flex align-items-center justify-content-center"
            ...
            },
            {
                ...
                width: 150,
                render: (ticket) => {
                    const config = STATUS_CONFIG[ticket.status as keyof typeof STATUS_CONFIG] || STATUS_CONFIG.closed
                    return (
                        <div className="d-flex align-items-center gap-2">
                            <div
                                className="rounded-circle d-flex align-items-center justify-content-center"
                        ...
                        <CardBody className="text-center">
                            ...
                            <div className="mb-2">
                                <div
                                    ...
                                    style={{ background: '#E8F5E9', color: '#2E7D32', borderRadius: '6px', fontSize: '0.75rem', fontWeight: 600 }}
                                >
                                    <span className="badge-dot" style={{ background: '#2E7D32', width: 6, height: 6, borderRadius: '50%' }}></span>
                                    Online 24/7
                                </div>
                            </div>
                ...
                <Modal.Body>
                  ...
                  <div className="fw-medium font-monospace small">{selectedTicket.id}</div>
                                ...
                                <Col md={6}>
                                    ...
                                    <div>
                                        {selectedTicket && (() => {
                                            const config = STATUS_CONFIG[selectedTicket.status as keyof typeof STATUS_CONFIG] || STATUS_CONFIG.closed
                                            return (
                                                <div className="d-flex align-items-center gap-2">
                                                    <div
                                                        className="rounded-circle d-flex align-items-center justify-content-center"
                                ...
                                <Col md={6}>
                                    ...
                                    <div>
                                        {selectedTicket && (() => {
                                            const config = TYPE_CONFIG[selectedTicket.type as keyof typeof TYPE_CONFIG] || TYPE_CONFIG.other
                                            return (
                                                <div className="d-flex align-items-center gap-2">
                                                    <div
                                                        className="rounded-circle d-flex align-items-center justify-content-center"
                                ...
                                <Col md={6}>
                                    ...
                                    <div>
                                        {selectedTicket && (() => {
                                            const config = PRIORITY_CONFIG[selectedTicket.priority as keyof typeof PRIORITY_CONFIG] || PRIORITY_CONFIG.low
                                            return (
                                                <div className="d-flex align-items-center gap-2">
                                                    {/* <div
                                                        className="rounded-circle d-flex align-items-center justify-content-center"
